...
// menu show mysql 
$sql = "SELECT * FROM `blog-name`";
$blogConnect = $connectionDB->query($sql);

// data add php to mysql 
if (isset($_POST['add'])) {
    $blogName = $_POST['blogName'];
    $addMysql = "INSERT INTO `blog-name`(`id`, `name`) VALUES (null,'$blogName')";
...
if (isset($_POST['delete'])) {
    ...
    $delItemSql = "DELETE FROM `blog-name` WHERE id=$delItem";
        ...
        <main class="flex-1 p-6 md:ml-[100%-256px]">
            <div class="container mx-auto">
                ...
                <form action="" method="post" class="flex items-center mb-4">
                    <input type="text" name="blogName" placeholder="Enter menu item" class="w-full px-4 py-2 border border-gray-300 rounded-l focus:outline-none focus:ring-2 focus:ring-blue-500" />
                ...
                <table class="min-w-full bg-white border border-gray-300">
                    ...
                    <tbody>
                        <?php
                        if ($blogConnect->num_rows > 0) {
                            while ($row = $blogConnect->fetch_assoc()) { ?>
                                <tr>
